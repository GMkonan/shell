...
QtObject {
    ...
    property string currentLayout: "en" // This is empty on startup. We could default it to "en", but we don't know the user's configured layout order (e.g. "en,ru" vs "ru,en").
    ...
    function handleRawEvent(event) {
        if (event.name === "activelayout") {
            ...
            const fullLayoutName = layoutInfo[layoutInfo.length - 1] === "Portuguese (Brazil)" ? "br" : layoutInfo[layoutInfo.length - 1];
